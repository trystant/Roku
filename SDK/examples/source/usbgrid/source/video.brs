...
' return a list of files with playable video extensions
	return get_filelist(filelist, "^.+\.(?:mp4|m4v|mov|wmv|ts)$")
end Function
...
Function make_video_content(url)
    ...
    if (extension = ".wmv")
	    content.streamformat = "wmv"
    endif
    if (extension = ".asf")
	    content.streamformat = "asf"
    endif
    return content
end Function 
...
Function newVideosContainer()
    container = newContentContainer("videos", findvideos, "icon-videos-hd.jpg", "icon-videos-sd.jpg", ShowVideo, videosettings, false)
    return container
end Function
