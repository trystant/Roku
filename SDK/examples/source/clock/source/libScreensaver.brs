...
Function CreateScreensaverCanvas(background_color = invalid, prt=invalid, loc_func=invalid, image_func=invalid)
    ...
    if (background_color <> invalid) then o.canvas.SetLayer(0, {Color:background_color, CompositionMode:"Source"})
...
Sub screensaverCanvas_Go() 
    print "screensaverCanvas_Go() called"
...
Function screensaverCanvas_Update(generateNewLoc = true)    
    ...
    m.canvas.SetLayer(1, image.Update(m.loc.x,m.loc.y))
End Function
